...
<tr style="background: transparent;">
  <td colspan="2" style="padding: 10px 0 6px 0; border: none; font-weight: bold; font-size: 12px; color: #1e293b; line-height: 1.2;">
    Пространственные особенности ведомственных кампусов<br>
...
function updateLegend() {
      ...
      const scaleControl = document.querySelector('.maplibregl-ctrl-bottom-left'); 
      ...
      if (scaleControl) {
        if (currentZoom < 6.8) {
            scaleControl.style.display = 'none'; // Прячем
        } else {
            scaleControl.style.display = 'block'; // Показываем
        }
      }
    ...
    map.on('mousemove', regionLayers, (e) => {
      if (e.features.length > 0 && map.getZoom() < 4.0) {
        ...
        const currentSource = e.features[0].source; 
...
<div style="position: relative; width: 201%; height: 80vh; min-height: 600px;">
  ...
  <div style="position: absolute; top: 10px; left: 10px; background: rgba(255,255,255,0.95); padding: 8px; border-radius: 6px; box-shadow: 0 2px 6px rgba(0,0,0,0.15); z-index: 10; font-family: 'Noto Sans', sans-serif; font-size: 12px;">
    ...
    <div style="margin-top: 10px; border-top: 1px solid #cbd5e1; padding-top: 6px;">
      ...
      <label style="display: block; cursor: pointer; margin-bottom: 4px;"><input type="checkbox" id="toggle-roads" checked> Пешеходные дороги</label>
...
const mins = [35, 30, 25, 20, 15, 10, 5];
  ...
  // Создание сплошной вертикальой цветовой шкалы (высота 7 * 22px = 154px)
  let colorBarHtml = '<div style="display: flex; flex-direction: column; border: 1px solid #333;">';
  mins.forEach((min) => {
    colorBarHtml += `<div style="width: 16px; height: 22px; background: ${walkColors[min]}; opacity: 0.5;"></div>`;
  });
  colorBarHtml += '</div>';

  // Создание блока подписей строго по стыкам плашек
  let labelsHtml = `
    <div style="display: flex; flex-direction: column; justify-content: space-between; height: 154px; margin-left: 10px; font-family: 'Noto Sans', sans-serif; font-size: 11px; color: #1e293b;">
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">Более</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">30</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">25</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">20</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">15</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">10</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">5</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">Менее</div>
    </div>
  `;

  legList.innerHTML = `
    <div style="display: flex; flex-direction: row; align-items: stretch; padding-top: 6px; padding-bottom: 6px;">
      ${colorBarHtml}
      ${labelsHtml}
    </div>
  `;
...
> *Дальневосточный университет – это форпост российского образования.* (В. Путин)
...
Уникальное географическое положение предопределяет научные приоритеты ДВФУ. Здесь активно развиваются направления, критически важные для будущего Дальнего Востока и всей планеты: **биомедицина, изучение Мирового океана и искусственный интеллект**.
...
const mins = [10, 7, 5, 2];
  ...
  let colorBarHtml = '<div style="display: flex; flex-direction: column; border: 1px solid #333;">';
  mins.forEach((min) => {
    colorBarHtml += `<div style="width: 16px; height: 24px; background: ${walkColors[min]}; opacity: 0.5;"></div>`;
  });
  colorBarHtml += '</div>';

  let labelsHtml = `
    <div style="display: flex; flex-direction: column; justify-content: space-between; height: 96px; margin-left: 10px; font-family: 'Noto Sans', sans-serif; font-size: 11px; color: #1e293b;">
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">Более</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">7</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">5</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">2</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">Менее</div>
    </div>
  `;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const mapMsuWalk = new maplibregl.Map({
    ...
    }, center: [37.5274, 55.7028], zoom: 13.5, minZoom: 12.5, maxZoom: 18, maxBounds: [ [37.4600, 55.6800], [37.5800, 55.7300] ]
  });
  ...
  document.querySelectorAll('input[name="iso-type-msu"]').forEach(r => r.addEventListener('change', (e) => { 
    ...
    const showPoints = document.getElementById('toggle-kpp-msu').checked; 
...
# Данные на основе атрибутивной таблицы слоя МГУ
data <- data.frame(
  FZ = c("academ", "cultural", "dormit", "green", "indust", "parking", "sport", "tech", "uslugi"),
  area_m2 = c(881606, 78220, 62700, 919894, 19060, 10943, 125483, 87766, 17679),
  share = c(40.01, 3.55, 2.85, 41.75, 0.86, 0.50, 5.69, 3.98, 0.81)
...
datatable(
  ...
  formatRound(columns = "area_m2", digits = 0) %>% 
  formatRound(columns = "share", digits = 2)
```
...
const mins = [15, 10, 5];
  ...
  let labelsHtml = `
    <div style="display: flex; flex-direction: column; justify-content: space-between; height: 72px; margin-left: 10px; font-family: 'Noto Sans', sans-serif; font-size: 11px; color: #1e293b;">
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">Более</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">10</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">5</div>
      <div style="height: 0; display: flex; align-items: center; white-space: nowrap;">Менее</div>
...
color_map <- c("academ"="#f23761", "dormits"="#ff807a", "green"="#abeca3", "hoz"="#6863db", "parking"="#a82072", "sport"="#ffa860", "uslugi"="#fff3a3")
labels <- c("academ"="Научно-образовательная", "dormits"="Жилая", "green"="Ландшафтно-рекреационная", "hoz"="Хозяйственная", "parking"="Парковочная", "sport"="Спортивная", "uslugi"="Зона услуг")

data$Label <- labels[data$FZ]
data$Color <- color_map[data$FZ]

datatable(
  data[, c("Label", "area_m2", "share")],
  colnames = c("Функциональная зона", "Площадь (м²)", "Доля (%)"),
  options = list(pageLength = 7, dom = 't', ordering = FALSE, scrollX = TRUE),
  rownames = FALSE
) %>% 
  formatRound(columns = "area_m2", digits = 0) %>% 
  formatRound(columns = "share", digits = 2)
